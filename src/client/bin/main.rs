...
enum ClientRequest {
    ...
    ConsensusState,
    // Get a Data Root Inclusion Proof.
    ...
    NetInfo,
    /// Get a SharesProof for a specific range
    ProveShares {
    ...
    C: Client + Sync,
{
    let result = match req {
        ...
        ClientRequest::DataRootInclusionProof { height, start, end } => {
            serde_json::to_string_pretty(
        ...
        ClientRequest::ProveShares {
            height,
            start_share,
            end_share,
        } => serde_json::to_string_pretty(
            &client.prove_shares(height, start_share, end_share).await?,
        )
        .map_err(Error::serde)?,
