...
pub mod vec_vec_base64string {
    ...
    /// Serialize from `Vec<Vec<u8>>` into `Vec<base64string>`
        ...
        S: Serializer,
    {
        let base64_strings = value
            .iter()
            .map(|v| {
                String::from_utf8(base64::encode(v))
                    .map_err(|e| serde::ser::Error::custom(format!("Base64 encode error: {}", e)))
            })
            .collect::<Result<Vec<String>, S::Error>>()?;
        serializer.serialize_some(&base64_strings)
    }
